    ...
    def _wait_for_response(self, timeout: int, existing_urls: Optional[Set[str]] = None) -> Tuple[List[str], str]:
        ...
        while time.time() - start < timeout:
            ...
            try:
                ...
                """)
                if text:
                    print(f"DEBUG: Found text: {text[:80]}...")  # Print more of the text so we can see it
                    if text == last_text:
                        text_stable_count += 1
                        if text_stable_count >= 4:  # <--- CHANGED TO 4 (waits ~8 seconds of no typing to confirm it's done)
                            return [], text
                    else:
                        text_stable_count = 0
                        last_text = text
